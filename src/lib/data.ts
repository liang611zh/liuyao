...
// ============================================================
//
// 纯数据 + 纯查表，不依赖 React、不依赖 DOM、不依赖 i18n。
// 这里的每一张表都有传统筮法的标准答案兜底，改动前请先看 test/paipan.test.ts。
...
//
// ⚠️ 位序写反了不会报错 —— 反转在八卦上是双射，查表照样查得到，
//    只是查成了另一个卦。历史上就栽过这一次，64 卦里 48 个显示成别的卦。
...
export const TRIGRAM_BY_NAME = Object.fromEntries(
  Object.entries(TRIGRAMS).map(([bin, tri]) => [tri.name, bin]),
) as Record<TrigramName, string>

// 八卦符号，用于卦象展示
export const TRIGRAM_SYMBOL: Record<TrigramName, string> = {
  乾: '☰', 兑: '☱', 离: '☲', 震: '☳',
  巽: '☴', 坎: '☵', 艮: '☶', 坤: '☷',
}
...
export const PALACES: Record<TrigramName, string[]> = {
...
// 宫内序号 -> 世次名称（本宫/一世/…/游魂/归魂）
export const PALACE_RANK = [
  '本宫', '一世', '二世', '三世', '四世', '五世', '游魂', '归魂',
] as const

// 卦名与上下卦对应关系
export const GUA_TRIGRAM_MAP: Record<string, { upper: TrigramName; lower: TrigramName }> = {
  乾为天: { upper: '乾', lower: '乾' },
  天风姤: { upper: '乾', lower: '巽' },
  天山遁: { upper: '乾', lower: '艮' },
  天地否: { upper: '乾', lower: '坤' },
...
// binary -> hexagram 查找表
// binary: 6 位字符串，从下到上（index 0 = 初爻, index 5 = 上爻）
export const HEXAGRAMS: Record<string, Hexagram> = {}
for (const [palace, guaList] of Object.entries(PALACES) as [TrigramName, string[]][]) {
  guaList.forEach((guaName, i) => {
    const { upper, lower } = GUA_TRIGRAM_MAP[guaName]
    // 6 位: 初爻…上爻 —— 下卦在前
    const binary = TRIGRAM_BY_NAME[lower] + TRIGRAM_BY_NAME[upper]
    HEXAGRAMS[binary] = {
      gua: guaName,
      palace,
      palaceIndex: i,
...
export const NAJIA_INNER: Record<TrigramName, Branch[]> = {
  乾: ['子', '寅', '辰'],
  坎: ['寅', '辰', '午'],
  艮: ['辰', '午', '申'],
  震: ['子', '寅', '辰'],
...
export const NAJIA_OUTER: Record<TrigramName, Branch[]> = {
  乾: ['午', '申', '戌'],
  坎: ['申', '戌', '子'],
  艮: ['戌', '子', '寅'],
  震: ['午', '申', '戌'],
...
export const NAJIA_STEM_INNER: Record<TrigramName, Stem> = {
  乾: '甲', 坎: '戊', 艮: '丙', 震: '庚',
  巽: '辛', 离: '己', 坤: '乙', 兑: '丁',
}

export const NAJIA_STEM_OUTER: Record<TrigramName, Stem> = {
  乾: '壬', 坎: '戊', 艮: '丙', 震: '庚',
  巽: '辛', 离: '己', 坤: '癸', 兑: '丁',
}
...
// 六神 (Six Spirits)
export const SIX_SPIRITS: Spirit[] = ['青龙', '朱雀', '勾陈', '螣蛇', '白虎', '玄武']

// 天干 -> 六神起始索引 (从初爻开始)
// 口诀：甲乙起青龙，丙丁起朱雀，戊日起勾陈，己日起螣蛇，庚辛起白虎，壬癸起玄武
export const SPIRIT_START: Record<Stem, number> = {
  甲: 0, 乙: 0, 丙: 1, 丁: 1, 戊: 2,
  己: 3, 庚: 4, 辛: 4, 壬: 5, 癸: 5,
}

// 五行相生: A 生 B
export const SHENG_CYCLE: Record<Element, Element> = {
  金: '水', 水: '木', 木: '火', 火: '土', 土: '金',
}

// 五行相克: A 克 B
export const KE_CYCLE: Record<Element, Element> = {
  金: '木', 木: '土', 土: '水', 水: '火', 火: '金',
}
